#!/usr/bin/env python3
"""
Admin Commands: User management, PIN-based authentication, Drive folder auto-creation.
"""
...
class AdminCommandHandler:
    """Handles admin-level operations: add user, delete user, list users."""
    ...
    def execute(self, intent: str, user_id: str, text: str, context: Dict[str, Any]) -> str:
        """Execute an admin command after PIN verification."""
        pin = context.get('pin', '')
        if not pin:
            words = text.split()
            for word in words:
        ...
        logger.info(f"Admin auth successful: {intent} from {user_id}")
    ...
    def add_user(self, text: str, admin_user_id: str) -> str:
        ...
        try:
            ...
            if not success:
                return f"Failed to create or share Drive folder for {target_line_id}."
    ...
    def batch_add_users(self, user_list: list) -> str:
        ...
        try:
            ...
            save_users(self.users)
            summary = f"Batch add complete: {len([r for r in results if 'added' in r])}/{len(user_list)} successful"
            logger.info(summary)
            return '\n'.join(results) + f"\n\n{summary}"
        except Exception as e:
...
def init_admin_pin(pin: str = '1234'):
    """Initialize admin PIN for first run."""
    pin_hash = hashlib.sha256(pin.encode()).hexdigest()
