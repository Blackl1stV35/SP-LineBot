#!/usr/bin/env python3
"""
Drive Scanner: Semantic CSV/Excel unrolling for inventory data.
Transforms 2D matrices into dense, searchable text chunks.
"""
...
def parse_dense_inventory_csv(filepath: str):
    """
    Transforms a 2D CSV/Excel matrix into dense, highly-searchable semantic text chunks.
    Formats exactly to match dashboard regex patterns.
    """
    chunks = []
    try:
        rows = []
        ...
        for row in rows[2:]:
            ...
            for col_idx in range(1, len(row)):
                if col_idx < len(headers):
                    ...
                    # ONLY record if quantity is valid
                    if qty and qty != "" and qty != "|" and qty.lower() != "nan":
                        items_drawn.append(f"{item_name} จำนวน {qty}")
            
            # 4. Format strictly for Dashboard Regex and ChromaDB
            if items_drawn:
                chunk_text = (
        ...
        logger.info(f"Parsed {len(chunks)} inventory chunks from {filepath}")
